...
public class TrackManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (m_TotalWorldDistance > currentTarget)
        {
            ...
            Analytics.CustomEvent("level_complete", new Dictionary<string, object> { { "level_index", PlayerData.instance.rank} });
#endif
        }
    ...
    protected Stack<TrackSegment> m_Segment = new Stack<TrackSegment>();
    protected TrackSegment m_Original;
    public void PoolSegment(int initialSize)
    {
       
        for (int i = 0; i < initialSize; i++)
        {
            int segmentUse = Random.Range(0, m_CurrentThemeData.zones[m_CurrentZone].prefabList.Length);
            TrackSegment segmentToUse = m_CurrentThemeData.zones[Random.Range(0, m_CurrentThemeData.zones.Length)].prefabList[segmentUse];
            ...
            m_Segment.Push(newSegment);
        }
    }
    public TrackSegment GetSegment(Vector3 pos, Quaternion quat)
    {
        ...
        TrackSegment ret = m_Segment.Count > 0 ? m_Segment.Pop() : TrackSegment.Instantiate(segmentToUse);
    ...
    public void FreeSegment(TrackSegment obj)
    {
    ...
    public void SpawnNewSegment()
    {
        ...
        for (int i = 0; i < m_CurrentThemeData.zones.Length; i++)
        {
            for (int j = 0; j < m_CurrentThemeData.zones[i].prefabList.Length; j++)
            {
                Debug.Log(m_CurrentThemeData.zones[i].prefabList[j].name);
            }
        }
        int segmentUse = Random.Range(0, m_CurrentThemeData.zones[m_CurrentZone].prefabList.Length);
        ...
        //TrackSegment newSegment = GetSegment(Vector3.zero, Quaternion.identity);
