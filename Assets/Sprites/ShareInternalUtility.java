...
public final class ShareInternalUtility {
    ...
    static class C07005 implements Mapper<Attachment, String> {
        C07005() {
        }
    ...
    static class C07038 implements PhotoJSONProcessor {
        C07038() {
        }
    ...
    public static List<String> getPhotoUrls(SharePhotoContent photoContent, final UUID appCallId) {
        if (photoContent != null) {
            ...
            if (photos != null) {
                ...
                List<String> attachmentUrls = Utility.map(attachments, new C07005());
    ...
    public static Bundle getTextureUrlBundle(ShareCameraEffectContent cameraEffectContent, UUID appCallId) {
        if (cameraEffectContent != null) {
            CameraEffectTextures textures = cameraEffectContent.getTextures();
            if (textures != null) {
                Bundle attachmentUrlsBundle = new Bundle();
                List<Attachment> attachments = new ArrayList();
                for (String key : textures.keySet()) {
                    Attachment attachment = getAttachment(appCallId, textures.getTextureUri(key), textures.getTextureBitmap(key));
                    attachments.add(attachment);
    ...
    public static JSONObject toJSONObjectForWeb(ShareOpenGraphContent shareOpenGraphContent) throws JSONException {
        return OpenGraphJSONUtility.toJSONObject(shareOpenGraphContent.getAction(), new C07038());
    }
    ...
    private static Attachment getAttachment(UUID callId, Uri uri, Bitmap bitmap) {
        if (bitmap != null) {
